...
@Slf4j
@Component
public class SpringCompensationHandler implements CasePushHandler {

    @Override
    public String name() {
        return "Spring理算结果推送处理器";
    }

    @Override
    public void handle(CaseContext context) throws Exception {
        log.info("Spring处理理算结果推送, 案件ID: {}", context.getCaseId());
        ...
        context.setAttribute("compensationAmount", 8000.0);
